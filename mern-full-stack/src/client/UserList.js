...
class UserList extends Component {
  ...
  handleDelete(userId) {
    ...
    axios
      ...
      .then((response) => {
        // if the deletion was successful then re-render the list of Techniques
        this.updateUsers();
      })
      .catch((error) => {
        console.log(error);
    ...
    return (
      <div className="is-fluid">
        ...
        <nav className="navbar">
          <nav className="h">
            <h1 className="navbar-item title is-1 has-text-link">
              List of Techniques
            </h1>
            <h3 className="navbar-item title is-3 has-text-link">
              Karate-Do Goju-Ryu
            </h3>
          </nav>
        ...
        <footer className="footer has-background-primary">
          <div className="content has-text-centered">
            <p className="has-text-white-bis">
              Adapted by
              <strong> Bruno mendanha.</strong> 2020.
            </p>
          </div>
        </footer>
      </div>
